    ...
    texmath.katex ||
      (t && 'object' == typeof t.engine
        ...
        : 'object' == typeof module
          ...
          : (texmath.katex = { renderToString: () => 'No math renderer found.' })));
  for (const t of n.inline)
    (a && 'outerSpace' in t && (t.outerSpace = !0),
      ...
      (e.renderer.rules[t.name] = (e, n) =>
        t.tmpl.replace(/\$1/, texmath.render(e[n].content, !!t.displayMode, o))));
...
((texmath.mergeDelimiters = function (e) {
  ...
  for (const e of t)
    e in texmath.rules &&
      (n.inline.push(...texmath.rules[e].inline), n.block.push(...texmath.rules[e].block));
  return n;
}),
  (texmath.inline = (e) =>
    function (t, n) {
  ...
  (texmath.rules = {
    brackets: {
      inline: [{ name: 'math_inline', rex: /\\\((.+?)\\\)/gy, tmpl: '<eq>$1</eq>', tag: '\\(' }],
      block: [
        {
    ...
    doxygen: {
      inline: [{ name: 'math_inline', rex: /\\f\$(.+?)\\f\$/gy, tmpl: '<eq>$1</eq>', tag: '\\f$' }],
      block: [
        {
          name: 'math_block_eqno',
          rex: /\\f\[([^]+?)\\f\]\s*?\(([^)\s]+?)\)/gmy,
    ...
    julia: {
      inline: [
        { name: 'math_inline', rex: /`{2}([^`]+?)`{2}/gy, tmpl: '<eq>$1</eq>', tag: '``' },
        {
          name: 'math_inline',
          rex: /\$((?:\S?)|(?:\S.*?\S))\$/gy,
          tmpl: '<eq>$1</eq>',
    ...
    kramdown: {
      inline: [{ name: 'math_inline', rex: /\${2}(.+?)\${2}/gy, tmpl: '<eq>$1</eq>', tag: '$$' }],
      block: [
        {
          name: 'math_block_eqno',
          rex: /\${2}([^$]+?)\${2}\s*?\(([^)\s]+?)\)/gmy,
